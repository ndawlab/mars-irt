...
RAW_DIR = os.path.join(ROOT_DIR, 'raw', 'longform')
DATA_DIR = os.path.join(ROOT_DIR, 'data', 'longform')
...
for f in files:
    ...
    cols = ['item_set','long_form','item','distractor','shape_set','choice','accuracy','rt',
            'all_loaded','minimum_resolution','browser_interactions']
    mars = mars[cols]
    
    ## Format columns.
    mars['rt'] = np.round(mars['rt'] * 1e-3, 3)
    mars['long_form'] = mars['long_form'].astype(int)
...
## Store item_id.
cols = ['subject', 'trial', 'item_set', 'long_form', 'item_id', 'item', 'distractor',
...
MARS = MARS.merge(params[['item','shape_set','item_id','distractor']], on=['item','shape_set','distractor'])
...
for f in files:
    ...
    if os.path.isfile(fdat):
        ...
        dd['mars_task'] = np.round(sum([f(d) for d in JSON if 'long_form' in d]) * 1e-3, 3)
