...
parameters {
    ...
    vector[NK]  alpha_2_pr;            // Standardized level-2 effects
...
transformed parameters {
    ...
    vector[NK] alpha = Phi_approx(c + (X2 * alpha_mu) + (sigma[3] * X3 * alpha_1_pr) + (sigma[4] * alpha_2_pr)) * 5;
...
model {
    ...
    target += std_normal_lpdf(alpha_2_pr);
