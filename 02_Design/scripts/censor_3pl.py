...
mu = np.zeros(N)
for n in range(N): mu[n] = 0.25 + (1-0.25) * inv_logit(alpha[K[n]] * theta[J[n]] - beta[K[n]])
...
## Load StanModel
StanModel = CmdStanModel(stan_file=os.path.join(ROOT_DIR, 'stan_models', '1pl.stan'))
...
## Compute likelihood of accuracy [p(y=1 | theta, beta)].
Y_hat = inv_logit(theta_hat[J] - beta_hat[K])
...
for thresh in thresholds:
    ...
    censor = np.logical_and(Y==1, Y_hat < thresh).astype(int)
    ...
    ## Assemble data.
    dd = dict(N=Nc, M1=M1, M2=M2, J=Jc+1, K=Kc+1, Y=Yc, X1=X1, X2=X2)
    ...
    df = DataFrame(dict(
        ...
        thresh = np.repeat(thresh, n_subj + 2 * n_item),
...
## Save.
fout = os.path.join(ROOT_DIR, 'stan_results', '%s_c3pl_s%0.3d.csv' %(stan_model, seed))
