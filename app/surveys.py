...
def surveys():
    ...
    ## Case 2: previous completion of surveys.
    elif (f'%s_surveys.json' %session['subId'] in os.listdir(session['data'])) or \
         ('surveys' in session and session['surveys'] == 'complete'):
    ...
    ## Case 3: first or repeat visit.
        ...
        session['surveys'] = 'start'
        write_metadata(session, ['surveys'], 'a')

        ## Present experiment.
        return render_template('surveys.html', workerId=session['workerId'], assignmentId=session['assignmentId'], hitId=session['hitId'], code_success=session['code_success'], code_reject=session['code_reject'])
...
@bp.route('/redirect_mars', methods = ['POST'])
def redirect_mars():
